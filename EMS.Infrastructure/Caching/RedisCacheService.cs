namespace EMS.Infrastructure.Caching;
...
/// Provides distributed caching across the application
...
public class RedisCacheService : ICacheService
{
    ...
    public async Task RemoveByPatternAsync(string pattern, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug("Removed {Count} cache keys matching pattern: {Pattern}", keys.Count(), pattern);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error removing cache keys matching pattern: {Pattern}", pattern);
        }
    }
